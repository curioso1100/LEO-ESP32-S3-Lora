...
class SX126X:
    ...
    def getRSSI(self):
        ...
        rssiPkt = int((packetStatus >> 16) & 0xFF)
    ...
    def getSignalRSSI(self):
        """Lee el RSSI de la señal útil (SignalRSSI) del último paquete recibido.

        El SX1262 reporta tres valores en packetStatus:
          - data[0]: RssiPkt (RSSI del canal, incluye ruido)
          - data[1]: SnrPkt (SNR en complemento a 2)
          - data[2]: SignalRssiPkt (RSSI de la señal útil, sin ruido de fondo)

        SignalRSSI es útil para diagnosticar si un paquete con RSSI "flojo"
        es realmente una señal débil o hay mucho ruido de fondo.

        Retorna: float, SignalRSSI en dBm (negativo).
        """
        packetStatus = self.getPacketStatus()
        signalRssiPkt = int(packetStatus & 0xFF)
        return -1.0 * signalRssiPkt / 2.0
